...
//     (\`new IndoorEnvironmentalMonitoringSDK(...)\` / \`IndoorEnvironmentalMonitoringSDK.test(...)\` become
//     \`IndoorEnvironmentalMonitoringSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IndoorEnvironmentalMonitoringSDK'
...
const TEST_SEED = {"entity":{"environmental_monitoring":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IndoorEnvironmentalMonitoringSDK(...)\` and any
// \`IndoorEnvironmentalMonitoringSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IndoorEnvironmentalMonitoringSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IndoorEnvironmentalMonitoringSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
