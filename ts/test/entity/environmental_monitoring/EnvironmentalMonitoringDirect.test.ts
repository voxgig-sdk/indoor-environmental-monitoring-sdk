...
describe('EnvironmentalMonitoringDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new IndoorEnvironmentalMonitoringSDK({
  ...
  test('direct-list-environmental_monitoring', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-environmental_monitoring', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INDOOR_ENVIRONMENTAL_MONITORING_TEST_ENVIRONMENTAL_MONITORING_ENTID']
