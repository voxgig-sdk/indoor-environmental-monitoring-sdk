...
class TestEnvironmentalMonitoringEntity:

    def test_should_create_instance(self):
        testsdk = IndoorEnvironmentalMonitoringSDK.test(None, None)
        ent = testsdk.EnvironmentalMonitoring(None)
    ...
    def test_should_stream(self):
        ...
        base = IndoorEnvironmentalMonitoringSDK.test(seed, None)
        seen = list(base.EnvironmentalMonitoring(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from indoorenvironmentalmonitoring_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = IndoorEnvironmentalMonitoringSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.EnvironmentalMonitoring(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _environmental_monitoring_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "environmental_monitoring." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set INDOOR_ENVIRONMENTAL_MONITORING_TEST_ENVIRONMENTAL_MONITORING_ENTID JSON to run live")
        client = setup["client"]
        ...
        environmental_monitoring_ref01_data = None
        if len(environmental_monitoring_ref01_data_raw) > 0:
...
def _environmental_monitoring_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/environmental_monitoring/EnvironmentalMonitoringTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("INDOOR_ENVIRONMENTAL_MONITORING_TEST_ENVIRONMENTAL_MONITORING_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("INDOOR_ENVIRONMENTAL_MONITORING_TEST_LIVE") == "TRUE":
        ...
        client = IndoorEnvironmentalMonitoringSDK(helpers.to_map(merged_opts))

    _live = env.get("INDOOR_ENVIRONMENTAL_MONITORING_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("INDOOR_ENVIRONMENTAL_MONITORING_TEST_EXPLAIN") == "TRUE",
        "live": _live,
