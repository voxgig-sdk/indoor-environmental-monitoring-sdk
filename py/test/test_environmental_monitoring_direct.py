...
class TestEnvironmentalMonitoringDirect:

    def test_should_direct_list_environmental_monitoring(self):
        setup = _environmental_monitoring_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-environmental_monitoring", "live" if setup["live"] else "unit")
        if _skip:
...
def _environmental_monitoring_direct_setup(mockres):
