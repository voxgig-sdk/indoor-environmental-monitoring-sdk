# IndoorEnvironmentalMonitoring SDK test runner
...
class IndoorEnvironmentalMonitoringTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IndoorEnvironmentalMonitoringTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IndoorEnvironmentalMonitoringTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IndoorEnvironmentalMonitoringTestRunner.getenv("INDOOR_ENVIRONMENTAL_MONITORING_TEST_LIVE")
        override = IndoorEnvironmentalMonitoringTestRunner.getenv("INDOOR_ENVIRONMENTAL_MONITORING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IndoorEnvironmentalMonitoringTestRunner.getenv(key)
        ...
        explain = IndoorEnvironmentalMonitoringTestRunner.getenv("INDOOR_ENVIRONMENTAL_MONITORING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if IndoorEnvironmentalMonitoringTestRunner._test_control is not None:
            return IndoorEnvironmentalMonitoringTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IndoorEnvironmentalMonitoringTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IndoorEnvironmentalMonitoringTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IndoorEnvironmentalMonitoringTestRunner.load_test_control()
...
def load_env_local():
    IndoorEnvironmentalMonitoringTestRunner.load_env_local()


def env_override(m):
    return IndoorEnvironmentalMonitoringTestRunner.env_override(m)


def entity_data(v):
    return IndoorEnvironmentalMonitoringTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IndoorEnvironmentalMonitoringTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IndoorEnvironmentalMonitoringTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IndoorEnvironmentalMonitoringTestRunner.load_test_control()


def live_client_options():
    return IndoorEnvironmentalMonitoringTestRunner.live_client_options()


def live_delay_ms():
    return IndoorEnvironmentalMonitoringTestRunner.live_delay_ms()
