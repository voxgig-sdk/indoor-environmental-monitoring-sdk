# IndoorEnvironmentalMonitoring Python SDK Reference

Complete API reference for the IndoorEnvironmentalMonitoring Python SDK.
...
#### `IndoorEnvironmentalMonitoringSDK.test(testopts=None, sdkopts=None)`
...
#### `EnvironmentalMonitoring(data=None)`

Create a new `EnvironmentalMonitoringEntity` instance. Pass `None` for no initial data.
...
```python
results, err = client.EnvironmentalMonitoring().list({})
```
...
Create a new `EnvironmentalMonitoringEntity` instance with the same options.
