...
The Python SDK for the IndoorEnvironmentalMonitoring API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.EnvironmentalMonitoring(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.IndoorEnvironmentalMonitoring(None).load(
    {"id": "test01"}, None
)
