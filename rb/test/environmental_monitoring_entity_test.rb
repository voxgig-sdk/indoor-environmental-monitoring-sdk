...
class EnvironmentalMonitoringEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = IndoorEnvironmentalMonitoringSDK.test(seed, nil)
    seen = base.EnvironmentalMonitoring(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.EnvironmentalMonitoring(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set INDOOR_ENVIRONMENTAL_MONITORING_TEST_ENVIRONMENTAL_MONITORING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def environmental_monitoring_basic_setup(extra)
  ...
  entid_env_raw = ENV["INDOOR_ENVIRONMENTAL_MONITORING_TEST_ENVIRONMENTAL_MONITORING_ENTID"]
