# EnvironmentalMonitoring direct test
...
class EnvironmentalMonitoringDirectTest < Minitest::Test
  def test_direct_list_environmental_monitoring
    setup = environmental_monitoring_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-environmental_monitoring", setup[:live] ? "live" : "unit")
    if _should_skip
...
def environmental_monitoring_direct_setup(mockres)
  ...
  if live
    ...
    client = IndoorEnvironmentalMonitoringSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
