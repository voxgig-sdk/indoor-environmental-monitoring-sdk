...
require_relative 'IndoorEnvironmentalMonitoring_types'
...
class IndoorEnvironmentalMonitoringSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = IndoorEnvironmentalMonitoringHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, IndoorEnvironmentalMonitoringFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    IndoorEnvironmentalMonitoringUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => IndoorEnvironmentalMonitoringError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue IndoorEnvironmentalMonitoringError => err
    ...
    if fetched.is_a?(Hash)
      status = IndoorEnvironmentalMonitoringHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = IndoorEnvironmentalMonitoringError.new(
        "graphql_error", "IndoorEnvironmentalMonitoringSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = IndoorEnvironmentalMonitoringSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
