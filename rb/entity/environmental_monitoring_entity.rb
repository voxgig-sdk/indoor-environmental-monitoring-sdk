...
class EnvironmentalMonitoringEntity
  def initialize(client, entopts = nil)
    ...
    @_name = "environmental_monitoring"
    @_client = client
  ...
  def make
    opts = @_entopts.dup
    EnvironmentalMonitoringEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [EnvironmentalMonitoringListMatch, Hash, nil] match filter (any subset of
  #   EnvironmentalMonitoring fields); defaults to nil, treated as an empty match that lists all.
