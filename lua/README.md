...
The Lua SDK for the IndoorEnvironmentalMonitoring API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:EnvironmentalMonitoring(nil):list(nil, nil)
...
local result, err = client:IndoorEnvironmentalMonitoring(nil):load(
  { id = "test01" }, nil
)
