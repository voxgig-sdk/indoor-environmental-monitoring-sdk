# indoor-environmental-monitoring-cli

boru-driven command-line client **and** interactive REPL for the IndoorEnvironmentalMonitoring
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/indoor-environmental-monitoring-cli)
make build

# 2. See usage (words, entities, env vars)
./indoor-environmental-monitoring-cli --help

# 3. Provide credentials once, via the environment
export INDOOR_ENVIRONMENTAL_MONITORING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./indoor-environmental-monitoring-cli list environmental_monitoring

# 5. Override the API base URL for a single call
INDOOR_ENVIRONMENTAL_MONITORING_BASE=https://api.example.com ./indoor-environmental-monitoring-cli list environmental_monitoring

# 6. No arguments -> interactive REPL
./indoor-environmental-monitoring-cli
indoor-environmental-monitoring> list environmental_monitoring
indoor-environmental-monitoring> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/indoor-environmental-monitoring-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/indoor-environmental-monitoring-cli list environmental_monitoring
   ```
...
Run with no arguments to open a REPL (prompt `indoor-environmental-monitoring>`). Each line is
evaluated as its own boru expression:

```text
$ ./indoor-environmental-monitoring-cli
indoor-environmental-monitoring> list environmental_monitoring
indoor-environmental-monitoring> /help
indoor-environmental-monitoring> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `environmental_monitoring`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/indoor-environmental-monitoring-cli`. |
...
(`./indoor-environmental-monitoring-cli <expr>`) and interactively (the REPL), and expressions compose the
