// Command IndoorEnvironmentalMonitoring-cli is a CLI + REPL for the IndoorEnvironmentalMonitoring SDK,
...
const prompt = "indoor-environmental-monitoring"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "environmental_monitoring"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      INDOOR_ENVIRONMENTAL_MONITORING_APIKEY (api key), INDOOR_ENVIRONMENTAL_MONITORING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: INDOOR_ENVIRONMENTAL_MONITORING_APIKEY carries the API key and
	// INDOOR_ENVIRONMENTAL_MONITORING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("INDOOR_ENVIRONMENTAL_MONITORING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("INDOOR_ENVIRONMENTAL_MONITORING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIndoorEnvironmentalMonitoringSDK(opts)
