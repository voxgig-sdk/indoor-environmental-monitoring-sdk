...
// IndoorEnvironmentalMonitoring SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = IndoorEnvironmentalMonitoringOmni::makeStructRunner(
    ...
    {
        return IndoorEnvironmentalMonitoringOmni::objmodel(IndoorEnvironmentalMonitoringOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return IndoorEnvironmentalMonitoringOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return IndoorEnvironmentalMonitoringOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(IndoorEnvironmentalMonitoringOmni::structfix($now, Struct::undef()))
                        !== json_encode(IndoorEnvironmentalMonitoringOmni::structfix($val, Struct::undef()))) {
