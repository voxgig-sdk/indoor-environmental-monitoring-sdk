...
// IndoorEnvironmentalMonitoring SDK feature test
//
...
class FtCtrl extends IndoorEnvironmentalMonitoringControl
{
...
class FtClient
{
    ...
    private IndoorEnvironmentalMonitoringUtility $utility;

    public function __construct(IndoorEnvironmentalMonitoringUtility $utility, array $options)
    {
    ...
    public function get_utility(): IndoorEnvironmentalMonitoringUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IndoorEnvironmentalMonitoringContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IndoorEnvironmentalMonitoringContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IndoorEnvironmentalMonitoringContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IndoorEnvironmentalMonitoringConfig::make_feature($name);
    ...
    private static function buildUrl(IndoorEnvironmentalMonitoringSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IndoorEnvironmentalMonitoringError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IndoorEnvironmentalMonitoringResponse($fetched);
        }
    ...
    private function fail(IndoorEnvironmentalMonitoringContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IndoorEnvironmentalMonitoringContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
