...
//    only - a typed IndoorEnvironmentalMonitoringContext there would make every ctx
...
final class IndoorEnvironmentalMonitoringOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IndoorEnvironmentalMonitoringContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new IndoorEnvironmentalMonitoringSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new IndoorEnvironmentalMonitoringResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new IndoorEnvironmentalMonitoringError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new IndoorEnvironmentalMonitoringResponse($clean['response']);
    ...
    public static function err_from_map($m): ?IndoorEnvironmentalMonitoringError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IndoorEnvironmentalMonitoringError(is_string($code) ? $code : '', $msg);
    }
