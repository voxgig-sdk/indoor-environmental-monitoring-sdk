...
class EnvironmentalMonitoringEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = IndoorEnvironmentalMonitoringSDK::test($seed, null);
        $seen = iterator_to_array($base->EnvironmentalMonitoring(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = IndoorEnvironmentalMonitoringConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = IndoorEnvironmentalMonitoringSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->EnvironmentalMonitoring(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set INDOOR_ENVIRONMENTAL_MONITORING_TEST_ENVIRONMENTAL_MONITORING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function environmental_monitoring_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("INDOOR_ENVIRONMENTAL_MONITORING_TEST_ENVIRONMENTAL_MONITORING_ENTID");
