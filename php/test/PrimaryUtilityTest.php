...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ...
        $ctx = new IndoorEnvironmentalMonitoringContext($ctxmap, null);
        ...
        if (isset($ctxmap['spec']) && is_array($ctxmap['spec'])) {
            $ctx->spec = new IndoorEnvironmentalMonitoringSpec($ctxmap['spec']);
        }

        // Handle result from JSON map
        if (isset($ctxmap['result']) && is_array($ctxmap['result'])) {
            $ctx->result = new IndoorEnvironmentalMonitoringResult($ctxmap['result']);
            if (isset($ctxmap['result']['err']) && is_array($ctxmap['result']['err'])) {
                $msg = $ctxmap['result']['err']['message'] ?? '';
                $ctx->result->err = new IndoorEnvironmentalMonitoringError('', $msg);
            }
        }

        // Handle response from JSON map
        if (isset($ctxmap['response']) && is_array($ctxmap['response'])) {
            $ctx->response = new IndoorEnvironmentalMonitoringResponse($ctxmap['response']);
    ...
    private static function fixctx(IndoorEnvironmentalMonitoringContext $ctx, IndoorEnvironmentalMonitoringSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?IndoorEnvironmentalMonitoringError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IndoorEnvironmentalMonitoringError($code, $msg);
    }
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = IndoorEnvironmentalMonitoringSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new IndoorEnvironmentalMonitoringOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = IndoorEnvironmentalMonitoringSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof IndoorEnvironmentalMonitoringError) ? $result->err->msg : (string)$result->err;
