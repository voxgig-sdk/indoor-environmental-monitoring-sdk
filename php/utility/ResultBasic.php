...
// IndoorEnvironmentalMonitoring SDK utility: result_basic

class IndoorEnvironmentalMonitoringResultBasic
{
    public static function call(IndoorEnvironmentalMonitoringContext $ctx): ?IndoorEnvironmentalMonitoringResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof IndoorEnvironmentalMonitoringError) ? $result->err->msg : (string)$result->err;
