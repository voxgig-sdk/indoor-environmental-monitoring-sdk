...
// IndoorEnvironmentalMonitoring SDK EnvironmentalMonitoring entity
...
class EnvironmentalMonitoringEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "environmental_monitoring";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new EnvironmentalMonitoringEntity($this->_client, $opts);
    }

    /**
     * @param EnvironmentalMonitoring|array $args EnvironmentalMonitoring data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IndoorEnvironmentalMonitoringHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return EnvironmentalMonitoring|array The current EnvironmentalMonitoring data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of EnvironmentalMonitoring fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IndoorEnvironmentalMonitoringHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of EnvironmentalMonitoring fields).
     */
     ...
     *
     * @param EnvironmentalMonitoringListMatch|array|null $reqmatch Match filter (any subset
     *   of EnvironmentalMonitoring fields) as an assoc-array; EnvironmentalMonitoringListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EnvironmentalMonitoring[]|array A list of EnvironmentalMonitoring items as assoc-arrays at
     *   the SDK boundary; throws IndoorEnvironmentalMonitoringError on failure (item-5 convention).
     */
