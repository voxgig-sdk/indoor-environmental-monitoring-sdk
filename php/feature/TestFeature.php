...
// IndoorEnvironmentalMonitoring SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class IndoorEnvironmentalMonitoringTestFeature extends IndoorEnvironmentalMonitoringBaseFeature
{
    ...
    public function init(IndoorEnvironmentalMonitoringContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (IndoorEnvironmentalMonitoringContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = IndoorEnvironmentalMonitoringParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (IndoorEnvironmentalMonitoringContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(IndoorEnvironmentalMonitoringContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = IndoorEnvironmentalMonitoringParam::call($ctx, $k);
