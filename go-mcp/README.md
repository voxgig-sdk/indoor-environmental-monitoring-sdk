# indoor-environmental-monitoring-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IndoorEnvironmentalMonitoring SDK as
two agent tools — `indoor-environmental-monitoring_list` and `indoor-environmental-monitoring_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/indoor-environmental-monitoring-mcp)
make build

# 2. Provide credentials via the environment
export INDOOR_ENVIRONMENTAL_MONITORING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user indoor-environmental-monitoring \
  -- /absolute/path/to/indoor-environmental-monitoring-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./indoor-environmental-monitoring-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// indoor-environmental-monitoring_list: first page of records
{ "entity": "environmental_monitoring" }
{ "entity": "environmental_monitoring", "query": { } }

// indoor-environmental-monitoring_load: one record by id
{ "entity": "environmental_monitoring", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/indoor-environmental-monitoring-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user indoor-environmental-monitoring \
     -- "$PWD"/dist/*/indoor-environmental-monitoring-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `indoor-environmental-monitoring_list` and `indoor-environmental-monitoring_load` tools now appear
   in new sessions. Ask the agent to *"list environmental_monitoring using indoor-environmental-monitoring"*
   and it calls `indoor-environmental-monitoring_list` with `{"entity":"environmental_monitoring"}`.
...
```sh
export INDOOR_ENVIRONMENTAL_MONITORING_APIKEY=sk_live_xxx            # API key
export INDOOR_ENVIRONMENTAL_MONITORING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `indoor-environmental-monitoring_list` tool
...
### Call the `indoor-environmental-monitoring_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `indoor-environmental-monitoring_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `indoor-environmental-monitoring_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `INDOOR_ENVIRONMENTAL_MONITORING_APIKEY` | API key sent with every request. |
| `INDOOR_ENVIRONMENTAL_MONITORING_BASE` | Optional override of the API base URL. |
...
```sh
./indoor-environmental-monitoring-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"indoor-environmental-monitoring_load","arguments":{"entity":"environmental_monitoring","query":{"id":1}}}}'
```
