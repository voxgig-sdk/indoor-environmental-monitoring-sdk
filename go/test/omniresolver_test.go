...
import (
	...
	omni "github.com/voxgig-sdk/indoor-environmental-monitoring-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IndoorEnvironmentalMonitoringError); is {
				out := map[string]any{
					"name":    "IndoorEnvironmentalMonitoringError",
...
func omniCtx(arg any, client *sdk.IndoorEnvironmentalMonitoringSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IndoorEnvironmentalMonitoringSDK); is {
			client = live
