...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("INDOORENVIRONMENTALMONITORING_TEST_LIVE") == "TRUE" ||
		os.Getenv("INDOORENVIRONMENTALMONITORING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("INDOORENVIRONMENTALMONITORING_TEST_EXPLAIN"); explain != "" {
