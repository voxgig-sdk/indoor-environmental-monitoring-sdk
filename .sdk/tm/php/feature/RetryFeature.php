...
// IndoorEnvironmentalMonitoring SDK retry feature
...
class IndoorEnvironmentalMonitoringRetryFeature extends IndoorEnvironmentalMonitoringBaseFeature
{
    ...
    public function init(IndoorEnvironmentalMonitoringContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IndoorEnvironmentalMonitoringContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IndoorEnvironmentalMonitoringContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IndoorEnvironmentalMonitoringContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
