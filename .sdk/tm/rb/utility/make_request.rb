...
module IndoorEnvironmentalMonitoringUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IndoorEnvironmentalMonitoringResponse.new({})
    ...
    if fetch_err
      ...
      response = IndoorEnvironmentalMonitoringResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IndoorEnvironmentalMonitoringResponse.new(fetched)
    else
