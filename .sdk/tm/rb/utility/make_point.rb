...
module IndoorEnvironmentalMonitoringUtilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(IndoorEnvironmentalMonitoringError)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      op.points.each do |p|
        point = p
        select_def = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = IndoorEnvironmentalMonitoringHelpers.to_map(VoxgigStruct.getprop(point, "select"))
